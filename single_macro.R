######################################################################################
#  Coverage probability of 100(1-alpha)% confidence interval for macro-averaged MCC  #
######################################################################################
...
mcc1_p <- expression(
  1/3*((p11*(1-p11-p12-p13-p21-p31)-(p12+p13)*(p21+p31))/
         sqrt((p11+p12+p13)*(p11+p21+p31)*(1-p11-p21-p31)*(1-p11-p12-p13))
       +(p22*(1-p22-p12-p32-p21-p23)-(p12+p32)*(p21+p23))/
         sqrt((p22+p12+p32)*(p22+p21+p23)*(1-p22-p21-p23)*(1-p22-p12-p32))
       +(p33*(1-p33-p23-p13-p32-p31)-(p23+p13)*(p32+p31))/
         sqrt((p33+p23+p13)*(p33+p32+p31)*(1-p33-p32-p31)*(1-p33-p23-p13)))
...
mcc2_f <- expression(1/2*log((1+
  ...
  1-1/3*((p11*(1-p11-p12-p13-p21-p31)-(p12+p13)*(p21+p31))/
           sqrt((p11+p12+p13)*(p11+p21+p31)*(1-p11-p21-p31)*(1-p11-p12-p13))
         +(p22*(1-p22-p12-p32-p21-p23)-(p12+p32)*(p21+p23))/
           sqrt((p22+p12+p32)*(p22+p21+p23)*(1-p22-p21-p23)*(1-p22-p12-p32))
         +(p33*(1-p33-p23-p13-p32-p31)-(p23+p13)*(p32+p31))/
...
## true value
tMCC1<- 1/3*((p11*(p22+p23+p32+p33)-(p12+p13)*(p21+p31))/
               sqrt((p11+p12+p13)*(p11+p21+p31)*(p12+p13+p22+p23+p32+p33)*(p21+p22+p23+p31+p32+p33))
             +(p22*(p11+p13+p31+p33)-(p12+p32)*(p21+p23))/
               sqrt((p22+p12+p32)*(p22+p21+p23)*(p11+p13+p31+p33+p12+p32)*(p11+p13+p31+p33+p21+p23))
             +(p33*(p11+p12+p21+p22)-(p23+p13)*(p32+p31))/
               sqrt((p33+p23+p13)*(p33+p32+p31)*(p11+p12+p21+p22+p23+p13)*(p11+p12+p21+p22+p31+p32)))
...
for(j in 1:length(n)){
  ...
  MCC1 <-  1/3*((a11*(a22+a23+a32+a33)-(a12+a13)*(a21+a31))/
                  sqrt((a11+a12+a13)*(a11+a21+a31)*(a12+a13+a22+a23+a32+a33)*(a21+a22+a23+a31+a32+a33))
                +(a22*(a11+a13+a31+a33)-(a12+a32)*(a21+a23))/
                  sqrt((a22+a12+a32)*(a22+a21+a23)*(a11+a13+a31+a33+a12+a32)*(a11+a13+a31+a33+a21+a23))
                +(a33*(a11+a12+a21+a22)-(a23+a13)*(a32+a31))/
                  sqrt((a33+a23+a13)*(a33+a32+a31)*(a11+a12+a21+a22+a23+a13)*(a11+a12+a21+a22+a31+a32)))
...
res<-cbind(res,time)
#write.csv(res,file = "D:/study/MCC/R code/excel/s1_macro.csv")